    ...
    <div class="offset-3 col-6 text-center">
        <div class="btn-group" data-toggle="buttons">
            ...
            <button class="btn btn-outline-dark <?php if ($headerInfo["curentTab"] == "closing") {
                echo "active";
            } ?>" type="button" onclick="location.href = '/servicesprojectclosing/<?php echo $idSpace."/".$headerInfo["projectId"] ?>'"><?php echo ServicesTranslator::Closing($lang) ?></button> 
            <button class="btn btn-outline-dark <?php if ($headerInfo["curentTab"] == "samplereturn") {
                echo "active";
            } ?>" type="button" onclick="location.href = '/servicesprojectsample/<?php echo $idSpace."/".$headerInfo["projectId"] ?>'"><?php echo ServicesTranslator::SamplesStock($lang) ?></button>
            <button class="btn btn-outline-dark <?php if ($headerInfo["curentTab"] == "kanban") {
                echo "active";
            } ?>" type="button" onclick="location.href = '/servicesprojects/kanban/<?php echo $idSpace."/".$headerInfo["projectId"] ?>'"><?php echo ServicesTranslator::KanbanBoard($lang) ?></button> 
            <button class="btn btn-outline-dark
                <?php if ($headerInfo["curentTab"] == "gantt") {
                    echo "active";
                } ?>"
                type="button"
                onclick="location.href = '/servicesprojectgantt/<?php echo $idSpace."/0/".$headerInfo["personInCharge"]."/".$headerInfo["projectId"] ?>'"><?php echo ServicesTranslator::Gantt($lang) ?>
            </button> 
