...
class OwnerForm extends FormGenerator
{
    ...
    public function render()
    {
        ...
        $userList = $modelUser->getActivesForSelect("name");
        $this->form->addSelect(
            'owner_id_user',
            AntibodiesTranslator::Owner($this->lang),
            $userList["names"],
            $userList["ids"]
        );
        ...
        $this->form->addSelect(
            'owner_disponible',
            AntibodiesTranslator::Disponible($this->lang),
            array('disponible', 'épuisé', 'récupéré par équipe'),
            array(1,2,3)
        );
