...
class Acii extends Model {
    ...
    public function getBySpace($id_space) {
        $sql = "select * from ac_aciis WHERE id_space=?";
     ...
     */
    public function getAciis($sortentry = 'id') {

        $sql = "select * from ac_aciis order by " . $sortentry . " ASC;";
    ...
    public function get($id) {
        ...
        $sql = "select * from ac_aciis where id=?";
    ...
    public function edit($id, $name, $id_space) {
        ...
        $sql = "update ac_aciis set nom=?, id_space=? where id=?";
    ...
    public function getNameFromId($id) {
        $sql = "select nom from ac_aciis where id=?";
        $unit = $this->runRequest($sql, array($id));
        if ($unit->rowCount() == 1) {
            $tmp = $unit->fetch();
            return $tmp[0];
