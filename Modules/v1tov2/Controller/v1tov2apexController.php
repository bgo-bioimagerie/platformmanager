...
class v1tov2apexController extends Controller {

    public function indexAction() {

        // ---------- SETTINGS ----------
        $dsn_old = 'mysql:host=localhost;dbname=sygrrif2_apex;charset=utf8';
        $login_old = "root";
        $pwd_old = "root";
        ...
        if ($importUserSygrrif) {
            ...
            echo "fn 9 <br/>";
            $this->importBookingPrices($pdo_old, $belongingMap, $resourcesMap);
            echo "fn 10 <br/>";
            //$this->importPackagePices($pdo_old, $belongingMap, $packagesMap);
            echo "fn 11 <br/>";
    ...
    protected function importUnits($pdo_old, $belongingMap) {
        ...
        foreach ($units_old as $uo) {
            $name = $uo["name"];
            $address = $uo["address"];
            $id_belonging = 1;//$belongingMap[$uo["id_belonging"]];
            $newID = $modelUnit->importUnit2($this->unitNameMapping($name), $address);
            $unitMap[$uo["id"]] = $newID;
        }
    ...
    public function importPackagePices($pdo_old, $belongingMap, $packagesMap) {
        ...
        foreach ($prices_old as $d) {
            if (isset($packagesMap[$d["id_package"]]) && isset($belongingMap[$d["id_pricing"]])) {
                $id_package = $packagesMap[$d["id_package"]];
                $id_belongings = $belongingMap[$d["id_pricing"]];
                $modelPrices->setPrice($id_package, $id_belongings, $d["price"]);
            }
        }
    }

    public function importBookingPrices($pdo_old, $belongingMap, $resourcesMap) {
        ...
        foreach ($prices_old as $d) {
            ...
            if (isset($resourcesMap[$d["id_resource"]]) && isset($belongingMap[$d["id_pricing"]])) {
                $id_resource = $resourcesMap[$d["id_resource"]];
                $id_belongings = $belongingMap[$d["id_pricing"]];
    ...
    public function importScheduling($pdo_old, $areasMap) {
        ...
        foreach ($resources_old as $d) {
            ...
            $model->edit($id, $is_monday, $is_tuesday, $is_wednesday, $is_thursday, $is_friday, $is_saturday, $is_sunday, $day_begin, $day_end, $size_bloc_resa, $booking_time_scale, $resa_time_setting, $default_color_id);
        }
    }
    ...
    public function importProject($pdo_old, $id_space, $usersMap, $mapInvoices, $servicesMap) {
        ...
        foreach ($entries_old as $d) {
            ...
            $comment = $d["comment"];
            $modelProject->addService($id_project, $id_service, $date, $quantity, $comment, $id_invoice);
        }
    }
