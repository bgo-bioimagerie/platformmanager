...
class BrPricing extends Model {

    public function __construct() {
        $this->tableName = "br_pricings";
    ...
    public function get($id) {
        $sql = "SELECT * FROM br_pricings WHERE id=?";
    ...
    public function set($id, $id_space, $name) {
        if ($id == 0) {
            $sql = 'INSERT INTO br_pricings (id_space, name) VALUES (?,?)';
            $this->runRequest($sql, array($id_space, $name));
            return $this->getDatabase()->lastInsertId();
        } else {
            $sql = 'UPDATE br_pricings SET id_space=?, name=? WHERE id=?';
    ...
    public function delete($id) {
        $sql = "DELETE FROM br_pricings WHERE id=?";
