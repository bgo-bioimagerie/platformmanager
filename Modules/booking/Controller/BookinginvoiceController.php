...
class BookinginvoiceController extends InvoiceAbstractController {
    ...
    public function editForm($id_item, $id_space, $id_invoice, $lang) {
        ...
        $form->addExternalButton(InvoicesTranslator::GeneratePdf($lang), "bookinginvoiceedit/" . $id_space . "/" . $id_invoice . "/1");
    ...
    protected function getResourcesList($id_space, $lang){
        ...
        foreach($resources as $r){
            $ids[] = $r["id"] . "_day";
            $names[] = $r["name"] . " " . BookingTranslator::Day($lang);
            if ($modelNightWe->isNight($r["id"])){
                $ids[] = $r["id"] . "_night";
                $names[] = $r["name"] . " " . BookingTranslator::night($lang);
            }
            if ($modelNightWe->isWe($r["id"])){
                $ids[] = $r["id"] . "_we";
                $names[] = $r["name"] . " " . BookingTranslator::WE($lang);
            }
    ...
    protected function invoice($id_space, $beginPeriod, $endPeriod, $id_unit, $id_resp, $number = "") {
        ...
        $details = BookingTranslator::Details($lang) . "=" . "bookinginvoicedetail/".$id_space."/".$invoice_id ;
        ...
        foreach ($contentArray as $content) {
            ...
            if (count($data) == 3) {
                ...
                if (count($idArray) == 2){
                    ...
                    if ($idDay == "day"){
                       $name .= " " . BookingTranslator::Day($lang); 
                    }
                    else if($idDay == "night"){
                        $name .= " " . BookingTranslator::night($lang); 
                    }
                    else if($idDay == "we"){
                        $name .= " " . BookingTranslator::WE($lang); 
                    }
                }
    ...
    public function detailsAction($id_space, $id_invoice){
        ...
        $table = new TableView();
        $table->setTitle(BookingTranslator::Details($lang) . ":" .$invoiceInfo["number"], 3);
        
        $headers = array("id" => BookingTranslator::Number($lang),
                         "recipient" => BookingTranslator::Recipient($lang),
                         "date_begin" => BookingTranslator::Date_Begin($lang),
                         "date_end" => BookingTranslator::Date_End($lang),
                         "package" => BookingTranslator::Package($lang)
            );
