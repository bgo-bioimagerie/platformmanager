...
class EsSaleItemInvoice extends Model {
    ...
    public function getitems($id_space, $id_sale) {
        $sql = "SELECT * FROM es_sale_invoice_items WHERE id_sale=? AND id_space=? AND deleted=0";
    ...
    public function set($id_space, $id, $id_sale, $id_batch, $quantity, $price = -1) {
        if ( !$this->exists($id_space, $id) ) {
            $sql = 'INSERT INTO es_sale_invoice_items (id_sale, id_batch, quantity, price, id_space) VALUES (?,?,?,?,?)';
            $this->runRequest($sql, array($id_sale, $id_batch, $quantity, $price, $id_space));
            return $this->getDatabase()->lastInsertId();
        } else {
            $sql = 'UPDATE es_sale_invoice_items SET id_sale=?, id_batch=?, quantity=?, price=? WHERE id=? AND id_space=? AND deleted=0';
            $this->runRequest($sql, array($id_sale, $id_batch, $quantity, $price, $id, $id_space));
        }
    }
    
    public function exists($id_space, $id){
        $sql = "SELECT id FROM es_sale_invoice_items WHERE id=? AND id_space=? AND deleted=0";
    ...
    public function delete($id_space, $id) {
        $sql = "UPDATE es_sale_invoice_items SET deleted=1,deleted_at=NOW() WHERE id=? AND id_space=?";
